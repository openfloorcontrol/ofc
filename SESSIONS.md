...
Phase 1 (structural refactor) is complete: Session is its own runtime entity, Floor is a live DOM with mutation primitives, Chat and Room are unified. Phase 2 (single-process persistence) is mostly complete: SessionStore + MemoryStore + JSONLStore land event persistence; UUIDs + `ofc sessions` give the CLI workflow; SessionMeta + resume warnings give hygiene; Session IDs were lifted out of stores into Floor itself (every Session is a UUID end-to-end, no more `"default"` sentinel). FloorStore (Step 6) is the only remaining Phase 2 item. The Postgres SessionStore landed early (Step 8a — leaked forward from Phase 3) to unlock the API/web target, and the blueprint now carries a `config:` section for runtime knobs (frontend, web, store, debug, log) — what historically lived on the command line. Phase 3 then shifts the primary surface from `ofc run` to `ofc serve` — REST mutation API, SQL-backed FloorStore, web UI as the main consumer. The CLI keeps working but becomes a thin client over the REST API rather than its own design surface. Phase 4 (per-agent context strategies) follows and plugs into the existing AgentContext / AgentTurn regardless of which surface is driving.
...
- [ ] **Step 7 — `ofc serve` + REST mutation API.**
  Long-running HTTP daemon. The mutation primitives from Phase 1 (AddAgent, AddFurniture, UpdateAgent, ...) become endpoints: `PATCH /api/v1/floors/X/agents/@hiro`, `POST /api/v1/floors`, etc. The existing API server already serves furniture MCP routes and session reads; Step 7 adds the write side and the daemon lifecycle. A useful intermediate milestone (driven by the Camera Shack) is `POST /api/v1/floors/X/sessions` — visitor-per-session for shared-floor deployments — which the postgres backend already supports at the storage layer.

- [x] **Step 8a — Postgres `SessionStore`.**
  Three tables (sessions, events, event_refs) + embedded migration runner; per-session monotonic Seq via row-locked `sessions.next_seq` inside the Append tx. Selected via `--db <dsn>` or blueprint `config.store.dsn`. Landed early as a prereq for any multi-process web target.

- [ ] **Step 8b — SQL-backed `FloorStore` (+ SQLite for embedded).**
  `FloorStore` is one row per Floor with a JSON column or structured columns. SQLite for embedded deployments where one Postgres feels too heavy. JSONL and the file `FloorStore` keep working for local single-process development — backends sit behind the same interfaces.
...
- **Config: profiles + user-level defaults.** The blueprint's `config:` section already carries the runtime knobs (frontend, web, store, debug, log). Two layers are noted but not yet built: (1) `profiles:` overlays inside the blueprint (e.g. `dev`/`prod` selected via `--profile` or `OFC_PROFILE`), and (2) `~/.ofc/defaults.yaml` for machine-wide fallbacks. When those land, precedence becomes: CLI flag > env > profile > config > defaults.yaml > built-in.
- **Blueprint-served custom frontend.** Extend the blueprint with a `frontend:` section pointing at a static directory (e.g. a built Vue or Svelte bundle). The API server mounts it alongside the existing `web/dist`, so a project ships with its own UI — chat widget shape, branding, landing scene. Doubles down on "the blueprint is the artifact" — agents, furniture, runtime config, *and* the visible UI all in one file (plus a folder of static assets). Camera Shack's text-adventure landing scene is the motivating example.
